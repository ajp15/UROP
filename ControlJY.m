...
function ControlJY(initialParameters,optimisation)
 ...
 %   initialParameters(12)=0;%as there is no killing due to the inflammation in any of the experiments
    ...
    %print optimised parameters to a csv file, append to the end of the file
    % if it already exists. First number in each row is the cost for those parameters
%     fname = sprintf('parameters_Opt_%s.txt', model);
%     fid = fopen(fname, 'a');
%     fprintf(fid, '\n%f', constants.cost);
%     for i=1:length(OptimisationOutput.parameters)
%         fprintf(fid, ',%f', OptimisationOutput.parameters(i));
%     end
%  
    tElapsed=toc(tStart)
